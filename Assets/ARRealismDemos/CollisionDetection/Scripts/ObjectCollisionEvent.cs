//-----------------------------------------------------------------------
// <copyright file="ObjectCollisionEvent.cs" company="Google">
//
...
{
    /// <summary>
    /// The canvas with a button of "place object".
    /// </summary>
    public Canvas CanvasWithPlaceButton;
    ...
    public override void Trigger(GameObject collidedObject, float collisionPercentage = 1f)
    {
        ...
        if (collisionPercentage > k_LowBound)
        {
            m_CollisionAnimationProgress = 0f;
            CanvasWithPlaceButton.GetComponent<Canvas>().enabled = false;
        }
    }
    ...
    public override void Stop(GameObject collidedObject, float collisionPercentage = 0)
    {
        if (collisionPercentage < k_LowBound)
        {
            ...
            foreach (var renderer in m_ManipulatedObject.GetComponentsInChildren<Renderer>())
            {
            ...
            CanvasWithPlaceButton.GetComponent<Canvas>().enabled = true;
        }
    }
